...
class Scene031(Scene):
    def construct(self):
        ...
        # ---------------------------------------------------------
        percept_frame = RoundedRectangle(width=4.8, height=2.75, corner_radius=0.16, stroke_width=3, stroke_color=COLOR_USER, fill_color=COLOR_USER, fill_opacity=0.05).shift(LEFT * 3.05 + DOWN * 0.05)
        ...
        slots = VGroup(
            create_entity_box('cup', 'cốc', color=COLOR_USER, width=1.25, height=1.05),
            create_entity_box('ball', 'bóng', color=COLOR_RESULT, width=1.25, height=1.05),
            create_entity_box('box', 'hộp', color=COLOR_MATH, width=1.25, height=1.05),
        ).arrange(RIGHT, buff=0.18).move_to(percept_frame)

        causal_frame = RoundedRectangle(width=4.8, height=2.75, corner_radius=0.16, stroke_width=3, stroke_color=COLOR_WARNING, fill_color=COLOR_WARNING, fill_opacity=0.05).shift(RIGHT * 3.05 + DOWN * 0.05)
        ...
        cause = create_entity_box('ball', 'nguyên nhân', color=COLOR_WARNING, width=1.55, height=1.1).move_to(causal_frame.get_left() + RIGHT*1.15 + DOWN*0.15)
        effect = create_entity_box('box', 'kết quả', color=COLOR_RESULT, width=1.55, height=1.1).move_to(causal_frame.get_right() + LEFT*1.15 + DOWN*0.15)
